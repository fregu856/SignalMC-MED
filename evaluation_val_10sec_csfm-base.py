...
save_dir_path = signalmcmed_dir_path + "/evaluation_val_10sec_csfm-base"
...
with open(signalmcmed_dir_path + "/extract-features_csfm-base_10sec_ecg.pkl", "rb") as f:
    features_10sec_ecg = pickle.load(f)
print(features_10sec_ecg.shape)
with open(signalmcmed_dir_path + "/extract-features_csfm-base_10sec_ppg.pkl", "rb") as f:
    features_10sec_ppg = pickle.load(f)
print(features_10sec_ppg.shape)
with open(signalmcmed_dir_path + "/extract-features_csfm-base_10sec_ecg_ppg.pkl", "rb") as f:
    features_10sec_ecg_ppg = pickle.load(f)
...
for i in range(len(preprocessed_csns)):
    if preprocessed_csns[i] in signalmcmed_csns:
        features_10sec_ecg_list.append(features_10sec_ecg[i])
        features_10sec_ppg_list.append(features_10sec_ppg[i])
        features_10sec_ecg_ppg_list.append(features_10sec_ecg_ppg[i])
        preprocessed_csns_filter.append(preprocessed_csns[i])
features_10sec_ecg = np.stack(features_10sec_ecg_list, axis=0)
features_10sec_ppg = np.stack(features_10sec_ppg_list, axis=0)
features_10sec_ecg_ppg = np.stack(features_10sec_ecg_ppg_list, axis=0)
...
for train_prop_i, train_prop in enumerate(train_props):
    ...
    for run_i in range(number_of_runs):
        ...
        train_ages = []
        train_features_list_10sec_ecg = []
        train_features_list_10sec_ppg = []
        train_features_list_10sec_ecg_ppg = []
        for i, csn in enumerate(preprocessed_csns):
            if csn in train_csns:
                ...
                train_features_list_10sec_ecg.append(features_10sec_ecg[i])
                train_features_list_10sec_ppg.append(features_10sec_ppg[i])
                train_features_list_10sec_ecg_ppg.append(features_10sec_ecg_ppg[i])
        train_features_10sec_ecg = np.stack(train_features_list_10sec_ecg, axis=0)
        train_features_10sec_ppg = np.stack(train_features_list_10sec_ppg, axis=0)
        train_features_10sec_ecg_ppg = np.stack(train_features_list_10sec_ecg_ppg, axis=0)
        print(len(train_ages))
        print(train_features_10sec_ecg.shape)
        print(train_features_10sec_ppg.shape)
        print(train_features_10sec_ecg_ppg.shape)

        val_ages = []
        val_features_list_10sec_ecg = []
        val_features_list_10sec_ppg = []
        val_features_list_10sec_ecg_ppg = []
        for i, csn in enumerate(preprocessed_csns):
            if csn in val_csns:
                age_values = visits_df[visits_df["CSN"] == csn]["Age"].values
                assert len(age_values) == 1
                val_ages.append(age_values[0])
                val_features_list_10sec_ecg.append(features_10sec_ecg[i])
                val_features_list_10sec_ppg.append(features_10sec_ppg[i])
                val_features_list_10sec_ecg_ppg.append(features_10sec_ecg_ppg[i])
        val_features_10sec_ecg = np.stack(val_features_list_10sec_ecg, axis=0)
        val_features_10sec_ppg = np.stack(val_features_list_10sec_ppg, axis=0)
        val_features_10sec_ecg_ppg = np.stack(val_features_list_10sec_ecg_ppg, axis=0)
        print(len(val_ages))
        print(val_features_10sec_ecg.shape)
        print(val_features_10sec_ppg.shape)
        print(val_features_10sec_ecg_ppg.shape)

        X_train_10sec_ecg = train_features_10sec_ecg
        X_train_10sec_ppg = train_features_10sec_ppg
        X_train_10sec_ecg_ppg = train_features_10sec_ecg_ppg
        X_train_10sec_ecg_ppg_mean = (X_train_10sec_ecg + X_train_10sec_ppg)/2.0
        y_train = np.array(train_ages)
        #######
        indices = np.arange(X_train_10sec_ecg.shape[0])
        train_inds = rng.choice(indices, size=int(train_prop*len(indices)), replace=True)
        X_train_10sec_ecg = X_train_10sec_ecg[train_inds]
        X_train_10sec_ppg = X_train_10sec_ppg[train_inds]
        X_train_10sec_ecg_ppg = X_train_10sec_ecg_ppg[train_inds]
        X_train_10sec_ecg_ppg_mean = X_train_10sec_ecg_ppg_mean[train_inds]
        y_train = y_train[train_inds]
        print(len(y_train))
        print(X_train_10sec_ecg.shape)
        print(X_train_10sec_ppg.shape)
        print(X_train_10sec_ecg_ppg.shape)
        print(X_train_10sec_ecg_ppg_mean.shape)

        X_val_10sec_ecg = val_features_10sec_ecg
        X_val_10sec_ppg = val_features_10sec_ppg
        X_val_10sec_ecg_ppg = val_features_10sec_ecg_ppg
        X_val_10sec_ecg_ppg_mean = (X_val_10sec_ecg + X_val_10sec_ppg)/2.0
        ...
        pearsons_10sec_ppg = []
        maes_10sec_ecg_ppg = []
        pearsons_10sec_ecg_ppg = []
        maes_10sec_ecg_ppg_mean = []
        pearsons_10sec_ecg_ppg_mean = []
        for alpha in alpha_values:
            print(alpha)

            pipe = make_pipeline(StandardScaler(), Ridge(alpha=alpha))
            pipe.fit(X_train_10sec_ecg, y_train)
            y_val_pred = pipe.predict(X_val_10sec_ecg)
            mae = np.mean(np.abs(y_val - y_val_pred))
            maes_10sec_ecg.append(mae)
            pearson_corr, _ = pearsonr(y_val_pred, y_val)
            pearsons_10sec_ecg.append(pearson_corr)

            pipe = make_pipeline(StandardScaler(), Ridge(alpha=alpha))
            pipe.fit(X_train_10sec_ppg, y_train)
            y_val_pred = pipe.predict(X_val_10sec_ppg)
            mae = np.mean(np.abs(y_val - y_val_pred))
            maes_10sec_ppg.append(mae)
            pearson_corr, _ = pearsonr(y_val_pred, y_val)
            pearsons_10sec_ppg.append(pearson_corr)

            pipe = make_pipeline(StandardScaler(), Ridge(alpha=alpha))
            pipe.fit(X_train_10sec_ecg_ppg, y_train)
            y_val_pred = pipe.predict(X_val_10sec_ecg_ppg)
            mae = np.mean(np.abs(y_val - y_val_pred))
            maes_10sec_ecg_ppg.append(mae)
            pearson_corr, _ = pearsonr(y_val_pred, y_val)
            pearsons_10sec_ecg_ppg.append(pearson_corr)

            pipe = make_pipeline(StandardScaler(), Ridge(alpha=alpha))
            pipe.fit(X_train_10sec_ecg_ppg_mean, y_train)
            y_val_pred = pipe.predict(X_val_10sec_ecg_ppg_mean)
            mae = np.mean(np.abs(y_val - y_val_pred))
            maes_10sec_ecg_ppg_mean.append(mae)
            pearson_corr, _ = pearsonr(y_val_pred, y_val)
            pearsons_10sec_ecg_ppg_mean.append(pearson_corr)

        best_idx_ecg = np.argmax(pearsons_10sec_ecg)
        best_alpha_ecg = alpha_values[best_idx_ecg]
        #
        best_idx_ppg = np.argmax(pearsons_10sec_ppg)
        best_alpha_ppg = alpha_values[best_idx_ppg]
        #
        best_idx_ecg_ppg = np.argmax(pearsons_10sec_ecg_ppg)
        best_alpha_ecg_ppg = alpha_values[best_idx_ecg_ppg]
        #
        best_idx_ecg_ppg_mean = np.argmax(pearsons_10sec_ecg_ppg_mean)
        ...
        plt.figure(1)
        plt.plot(alpha_values, pearsons_10sec_ecg, "^-", color="C0", label="ECG-only -- 10sec")
        plt.plot(alpha_values, pearsons_10sec_ppg, "^-", color="C1", label="PPG-only -- 10sec")
        plt.plot(alpha_values, pearsons_10sec_ecg_ppg, "^-", color="C2", label="ECG + PPG -- 10sec")
        plt.plot(alpha_values, pearsons_10sec_ecg_ppg_mean, "^-", color="C3", label="ECG-only + PPG-only (mean of features) -- 10sec")
        plt.ylabel("Pearson corr")
        plt.xlabel("alpha")
        plt.xscale("log")
        plt.legend()
        plt.savefig("%s/Ridge_val_pearsons_combined_10sec_age_alpha_values_train_prop%.2f_run_i%d.png" % (save_dir_path, train_prop, run_i), dpi=300)
        plt.close(1)
        ...
        auprcs_10sec_ppg = []
        rocaucs_10sec_ppg = []
        auprcs_10sec_ecg_ppg = []
        rocaucs_10sec_ecg_ppg = []
        auprcs_10sec_ecg_ppg_mean = []
        rocaucs_10sec_ecg_ppg_mean = []
        for c_value in c_values:
            ...
            pipe.fit(X_train_10sec_ecg, y_train)
            y_val_pred = pipe.predict(X_val_10sec_ecg)
            y_val_proba = pipe.predict_proba(X_val_10sec_ecg)[:, 1]
            rocauc = roc_auc_score(y_val, y_val_proba)
            rocaucs_10sec_ecg.append(rocauc)
            auprc = average_precision_score(y_val, y_val_proba)
            auprcs_10sec_ecg.append(auprc)

            pipe = make_pipeline(StandardScaler(), LogisticRegression(C=c_value, penalty="l2", solver="lbfgs", max_iter=2000))
            pipe.fit(X_train_10sec_ppg, y_train)
            y_val_pred = pipe.predict(X_val_10sec_ppg)
            y_val_proba = pipe.predict_proba(X_val_10sec_ppg)[:, 1]
            rocauc = roc_auc_score(y_val, y_val_proba)
            rocaucs_10sec_ppg.append(rocauc)
            auprc = average_precision_score(y_val, y_val_proba)
            auprcs_10sec_ppg.append(auprc)

            pipe = make_pipeline(StandardScaler(), LogisticRegression(C=c_value, penalty="l2", solver="lbfgs", max_iter=2000))
            pipe.fit(X_train_10sec_ecg_ppg, y_train)
            y_val_pred = pipe.predict(X_val_10sec_ecg_ppg)
            y_val_proba = pipe.predict_proba(X_val_10sec_ecg_ppg)[:, 1]
            rocauc = roc_auc_score(y_val, y_val_proba)
            rocaucs_10sec_ecg_ppg.append(rocauc)
            auprc = average_precision_score(y_val, y_val_proba)
            auprcs_10sec_ecg_ppg.append(auprc)

            pipe = make_pipeline(StandardScaler(), LogisticRegression(C=c_value, penalty="l2", solver="lbfgs", max_iter=2000))
            pipe.fit(X_train_10sec_ecg_ppg_mean, y_train)
            y_val_pred = pipe.predict(X_val_10sec_ecg_ppg_mean)
            y_val_proba = pipe.predict_proba(X_val_10sec_ecg_ppg_mean)[:, 1]
            rocauc = roc_auc_score(y_val, y_val_proba)
            rocaucs_10sec_ecg_ppg_mean.append(rocauc)
            auprc = average_precision_score(y_val, y_val_proba)
            auprcs_10sec_ecg_ppg_mean.append(auprc)

        best_idx_ecg = np.argmax(rocaucs_10sec_ecg)
        best_c_ecg = c_values[best_idx_ecg]
        #
        best_idx_ppg = np.argmax(rocaucs_10sec_ppg)
        best_c_ppg = c_values[best_idx_ppg]
        #
        best_idx_ecg_ppg = np.argmax(rocaucs_10sec_ecg_ppg)
        best_c_ecg_ppg = c_values[best_idx_ecg_ppg]
        #
        best_idx_ecg_ppg_mean = np.argmax(rocaucs_10sec_ecg_ppg_mean)
        ...
        plt.figure(1)
        plt.plot(c_values, rocaucs_10sec_ecg, "^-", color="C0", label="ECG-only -- 10sec")
        plt.plot(c_values, rocaucs_10sec_ppg, "^-", color="C1", label="PPG-only -- 10sec")
        plt.plot(c_values, rocaucs_10sec_ecg_ppg, "^-", color="C2", label="ECG + PPG -- 10sec")
        plt.plot(c_values, rocaucs_10sec_ecg_ppg_mean, "^-", color="C3", label="ECG-only + PPG-only (mean of features) -- 10sec")
        plt.ylabel("AUROC")
        plt.xlabel("C")
        plt.xscale("log")
        plt.legend()
        plt.savefig("%s/LogisticRegression_val_rocaucs_combined_10sec_sex_c_values_train_prop%.2f_run_i%d.png" % (save_dir_path, train_prop, run_i), dpi=300)
        plt.close(1)
        ...
        print("10sec -- alpha: %f -- ECG-only -- Pearson: %.3f, MAE: %.3f" % (best_alpha_ecg, pearson_corr, mae))
        ...
        print("10sec -- alpha: %f -- PPG-only -- Pearson: %.3f, MAE: %.3f" % (best_alpha_ppg, pearson_corr, mae))
        ...
        print("10sec -- alpha: %f -- ECG + PPG -- Pearson: %.3f, MAE: %.3f" % (best_alpha_ecg_ppg, pearson_corr, mae))
        ...
        print("10sec -- alpha: %f -- ECG-only + PPG-only (mean of features) -- Pearson: %.3f, MAE: %.3f" % (best_alpha_ecg_ppg_mean, pearson_corr, mae))
        ...
        print("10sec -- C: %f -- ECG-only -- AUROC: %.3f, AUPRC: %.3f" % (best_c_ecg, rocauc, auprc))
        ...
        print("10sec -- C: %f -- PPG-only -- AUROC: %.3f, AUPRC: %.3f" % (best_c_ppg, rocauc, auprc))
        ...
        print("10sec -- C: %f -- ECG + PPG -- AUROC: %.3f, AUPRC: %.3f" % (best_c_ecg_ppg, rocauc, auprc))
        ...
        print("10sec -- C: %f -- ECG-only + PPG-only (mean of features) -- AUROC: %.3f, AUPRC: %.3f" % (best_c_ecg_ppg_mean, rocauc, auprc))
        ...
        #
        print("10sec -- alpha: %f -- Mean over all 9 regression tasks -- ECG-only -- Pearson: %.3f, MAE: %.3f" % (best_alpha_ecg, np.mean(all_tasks_pearson_values_ecg), np.mean(all_tasks_mae_values_ecg)))
        print("10sec -- alpha: %f -- Mean over all 9 regression tasks -- PPG-only -- Pearson: %.3f, MAE: %.3f" % (best_alpha_ppg, np.mean(all_tasks_pearson_values_ppg), np.mean(all_tasks_mae_values_ppg)))
        print("10sec -- alpha: %f -- Mean over all 9 regression tasks -- ECG + PPG -- Pearson: %.3f, MAE: %.3f" % (best_alpha_ecg_ppg, np.mean(all_tasks_pearson_values_ecg_ppg), np.mean(all_tasks_mae_values_ecg_ppg)))
        print("10sec -- alpha: %f -- Mean over all 9 regression tasks -- ECG-only + PPG-only (mean of features) -- Pearson: %.3f, MAE: %.3f" % (best_alpha_ecg_ppg_mean, np.mean(all_tasks_pearson_values_ecg_ppg_mean), np.mean(all_tasks_mae_values_ecg_ppg_mean)))
        ...
        #
        print("10sec -- alpha: %f -- Mean over the 8 labs regression tasks -- ECG-only -- Pearson: %.3f, MAE: %.3f" % (best_alpha_ecg, np.mean(labs_tasks_pearson_values_ecg), np.mean(labs_tasks_mae_values_ecg)))
        print("10sec -- alpha: %f -- Mean over the 8 labs regression tasks -- PPG-only -- Pearson: %.3f, MAE: %.3f" % (best_alpha_ppg, np.mean(labs_tasks_pearson_values_ppg), np.mean(labs_tasks_mae_values_ppg)))
        print("10sec -- alpha: %f -- Mean over the 8 labs regression tasks -- ECG + PPG -- Pearson: %.3f, MAE: %.3f" % (best_alpha_ecg_ppg, np.mean(labs_tasks_pearson_values_ecg_ppg), np.mean(labs_tasks_mae_values_ecg_ppg)))
        print("10sec -- alpha: %f -- Mean over the 8 labs regression tasks -- ECG-only + PPG-only (mean of features) -- Pearson: %.3f, MAE: %.3f" % (best_alpha_ecg_ppg_mean, np.mean(labs_tasks_pearson_values_ecg_ppg_mean), np.mean(labs_tasks_mae_values_ecg_ppg_mean)))
        ...
        #
        print("10sec -- C: %f -- Mean over all 11 classification tasks -- ECG-only -- AUROC: %.3f, AUPRC: %.3f" % (best_c_ecg, np.mean(all_tasks_auroc_values_ecg), np.mean(all_tasks_auprc_values_ecg)))
        print("10sec -- C: %f -- Mean over all 11 classification tasks -- PPG-only -- AUROC: %.3f, AUPRC: %.3f" % (best_c_ppg, np.mean(all_tasks_auroc_values_ppg), np.mean(all_tasks_auprc_values_ppg)))
        print("10sec -- C: %f -- Mean over all 11 classification tasks -- ECG + PPG -- AUROC: %.3f, AUPRC: %.3f" % (best_c_ecg_ppg, np.mean(all_tasks_auroc_values_ecg_ppg), np.mean(all_tasks_auprc_values_ecg_ppg)))
        print("10sec -- C: %f -- Mean over all 11 classification tasks -- ECG-only + PPG-only (mean of features) -- AUROC: %.3f, AUPRC: %.3f" % (best_c_ecg_ppg_mean, np.mean(all_tasks_auroc_values_ecg_ppg_mean), np.mean(all_tasks_auprc_values_ecg_ppg_mean)))
        ...
        #
        print("10sec -- C: %f -- Mean over the 9 ICD-10 code classification tasks -- ECG-only -- AUROC: %.3f, AUPRC: %.3f" % (best_c_ecg, np.mean(ICD10_tasks_auroc_values_ecg), np.mean(ICD10_tasks_auprc_values_ecg)))
        print("10sec -- C: %f -- Mean over the 9 ICD-10 code classification tasks -- PPG-only -- AUROC: %.3f, AUPRC: %.3f" % (best_c_ppg, np.mean(ICD10_tasks_auroc_values_ppg), np.mean(ICD10_tasks_auprc_values_ppg)))
        print("10sec -- C: %f -- Mean over the 9 ICD-10 code classification tasks -- ECG + PPG -- AUROC: %.3f, AUPRC: %.3f" % (best_c_ecg_ppg, np.mean(ICD10_tasks_auroc_values_ecg_ppg), np.mean(ICD10_tasks_auprc_values_ecg_ppg)))
        print("10sec -- C: %f -- Mean over the 9 ICD-10 code classification tasks -- ECG-only + PPG-only (mean of features) -- AUROC: %.3f, AUPRC: %.3f" % (best_c_ecg_ppg_mean, np.mean(ICD10_tasks_auroc_values_ecg_ppg_mean), np.mean(ICD10_tasks_auprc_values_ecg_ppg_mean)))
    ...
    print("########")
    print("10sec -- 1. Age regression -- ECG-only -- Pearson: %.3f +/- %.4f -- MAE: %.3f +/- %.4f" % (np.mean(age_pearson_values_ecg), np.std(age_pearson_values_ecg), np.mean(age_mae_values_ecg), np.std(age_mae_values_ecg)))
    print("10sec -- 1. Age regression -- PPG-only -- Pearson: %.3f +/- %.4f -- MAE: %.3f +/- %.4f" % (np.mean(age_pearson_values_ppg), np.std(age_pearson_values_ppg), np.mean(age_mae_values_ppg), np.std(age_mae_values_ppg)))
    print("10sec -- 1. Age regression -- ECG + PPG -- Pearson: %.3f +/- %.4f -- MAE: %.3f +/- %.4f" % (np.mean(age_pearson_values_ecg_ppg), np.std(age_pearson_values_ecg_ppg), np.mean(age_mae_values_ecg_ppg), np.std(age_mae_values_ecg_ppg)))
    print("10sec -- 1. Age regression -- ECG-only + PPG-only (mean of features) -- Pearson: %.3f +/- %.4f -- MAE: %.3f +/- %.4f" % (np.mean(age_pearson_values_ecg_ppg_mean), np.std(age_pearson_values_ecg_ppg_mean), np.mean(age_mae_values_ecg_ppg_mean), np.std(age_mae_values_ecg_ppg_mean)))

    print("########")
    print("10sec -- 2. Sex classification -- ECG-only -- AUROC: %.3f +/- %.4f -- AUPRC: %.3f +/- %.4f" % (np.mean(sex_auroc_values_ecg), np.std(sex_auroc_values_ecg), np.mean(sex_auprc_values_ecg), np.std(sex_auprc_values_ecg)))
    print("10sec -- 2. Sex classification -- PPG-only -- AUROC: %.3f +/- %.4f -- AUPRC: %.3f +/- %.4f" % (np.mean(sex_auroc_values_ppg), np.std(sex_auroc_values_ppg), np.mean(sex_auprc_values_ppg), np.std(sex_auprc_values_ppg)))
    print("10sec -- 2. Sex classification -- ECG + PPG -- AUROC: %.3f +/- %.4f -- AUPRC: %.3f +/- %.4f" % (np.mean(sex_auroc_values_ecg_ppg), np.std(sex_auroc_values_ecg_ppg), np.mean(sex_auprc_values_ecg_ppg), np.std(sex_auprc_values_ecg_ppg)))
    print("10sec -- 2. Sex classification -- ECG-only + PPG-only (mean of features) -- AUROC: %.3f +/- %.4f -- AUPRC: %.3f +/- %.4f" % (np.mean(sex_auroc_values_ecg_ppg_mean), np.std(sex_auroc_values_ecg_ppg_mean), np.mean(sex_auprc_values_ecg_ppg_mean), np.std(sex_auprc_values_ecg_ppg_mean)))

    print("########")
    print("10sec -- 3. ED_dispo classification -- ECG-only -- AUROC: %.3f +/- %.4f -- AUPRC: %.3f +/- %.4f" % (np.mean(dispo_auroc_values_ecg), np.std(dispo_auroc_values_ecg), np.mean(dispo_auprc_values_ecg), np.std(dispo_auprc_values_ecg)))
    print("10sec -- 3. ED_dispo classification -- PPG-only -- AUROC: %.3f +/- %.4f -- AUPRC: %.3f +/- %.4f" % (np.mean(dispo_auroc_values_ppg), np.std(dispo_auroc_values_ppg), np.mean(dispo_auprc_values_ppg), np.std(dispo_auprc_values_ppg)))
    print("10sec -- 3. ED_dispo classification -- ECG + PPG -- AUROC: %.3f +/- %.4f -- AUPRC: %.3f +/- %.4f" % (np.mean(dispo_auroc_values_ecg_ppg), np.std(dispo_auroc_values_ecg_ppg), np.mean(dispo_auprc_values_ecg_ppg), np.std(dispo_auprc_values_ecg_ppg)))
    print("10sec -- 3. ED_dispo classification -- ECG-only + PPG-only (mean of features) -- AUROC: %.3f +/- %.4f -- AUPRC: %.3f +/- %.4f" % (np.mean(dispo_auroc_values_ecg_ppg_mean), np.std(dispo_auroc_values_ecg_ppg_mean), np.mean(dispo_auprc_values_ecg_ppg_mean), np.std(dispo_auprc_values_ecg_ppg_mean)))

    print("########")
    print("10sec -- Mean over all 11 classification tasks -- ECG-only -- AUROC: %.3f +/- %.4f -- AUPRC: %.3f +/- %.4f" % (np.mean(all11clstasks_auroc_values_ecg), np.std(all11clstasks_auroc_values_ecg), np.mean(all11clstasks_auprc_values_ecg), np.std(all11clstasks_auprc_values_ecg)))
    print("10sec -- Mean over all 11 classification tasks -- PPG-only -- AUROC: %.3f +/- %.4f -- AUPRC: %.3f +/- %.4f" % (np.mean(all11clstasks_auroc_values_ppg), np.std(all11clstasks_auroc_values_ppg), np.mean(all11clstasks_auprc_values_ppg), np.std(all11clstasks_auprc_values_ppg)))
    print("10sec -- Mean over all 11 classification tasks -- ECG + PPG -- AUROC: %.3f +/- %.4f -- AUPRC: %.3f +/- %.4f" % (np.mean(all11clstasks_auroc_values_ecg_ppg), np.std(all11clstasks_auroc_values_ecg_ppg), np.mean(all11clstasks_auprc_values_ecg_ppg), np.std(all11clstasks_auprc_values_ecg_ppg)))
    print("10sec -- Mean over all 11 classification tasks -- ECG-only + PPG-only (mean of features) -- AUROC: %.3f +/- %.4f -- AUPRC: %.3f +/- %.4f" % (np.mean(all11clstasks_auroc_values_ecg_ppg_mean), np.std(all11clstasks_auroc_values_ecg_ppg_mean), np.mean(all11clstasks_auprc_values_ecg_ppg_mean), np.std(all11clstasks_auprc_values_ecg_ppg_mean)))

    print("########")
    print("10sec -- Mean over the 9 ICD-10 code classification tasks -- ECG-only -- AUROC: %.3f +/- %.4f -- AUPRC: %.3f +/- %.4f" % (np.mean(icd10codetasks_auroc_values_ecg), np.std(icd10codetasks_auroc_values_ecg), np.mean(icd10codetasks_auprc_values_ecg), np.std(icd10codetasks_auprc_values_ecg)))
    print("10sec -- Mean over the 9 ICD-10 code classification tasks -- PPG-only -- AUROC: %.3f +/- %.4f -- AUPRC: %.3f +/- %.4f" % (np.mean(icd10codetasks_auroc_values_ppg), np.std(icd10codetasks_auroc_values_ppg), np.mean(icd10codetasks_auprc_values_ppg), np.std(icd10codetasks_auprc_values_ppg)))
    print("10sec -- Mean over the 9 ICD-10 code classification tasks -- ECG + PPG -- AUROC: %.3f +/- %.4f -- AUPRC: %.3f +/- %.4f" % (np.mean(icd10codetasks_auroc_values_ecg_ppg), np.std(icd10codetasks_auroc_values_ecg_ppg), np.mean(icd10codetasks_auprc_values_ecg_ppg), np.std(icd10codetasks_auprc_values_ecg_ppg)))
    print("10sec -- Mean over the 9 ICD-10 code classification tasks -- ECG-only + PPG-only (mean of features) -- AUROC: %.3f +/- %.4f -- AUPRC: %.3f +/- %.4f" % (np.mean(icd10codetasks_auroc_values_ecg_ppg_mean), np.std(icd10codetasks_auroc_values_ecg_ppg_mean), np.mean(icd10codetasks_auprc_values_ecg_ppg_mean), np.std(icd10codetasks_auprc_values_ecg_ppg_mean)))

    print("########")
    print("10sec -- Mean over all 9 regression tasks -- ECG-only -- Pearson: %.3f +/- %.4f -- MAE: %.3f +/- %.4f" % (np.mean(all9regtasks_pearson_values_ecg), np.std(all9regtasks_pearson_values_ecg), np.mean(all9regtasks_mae_values_ecg), np.std(all9regtasks_mae_values_ecg)))
    print("10sec -- Mean over all 9 regression tasks -- PPG-only -- Pearson: %.3f +/- %.4f -- MAE: %.3f +/- %.4f" % (np.mean(all9regtasks_pearson_values_ppg), np.std(all9regtasks_pearson_values_ppg), np.mean(all9regtasks_mae_values_ppg), np.std(all9regtasks_mae_values_ppg)))
    print("10sec -- Mean over all 9 regression tasks -- ECG + PPG -- Pearson: %.3f +/- %.4f -- MAE: %.3f +/- %.4f" % (np.mean(all9regtasks_pearson_values_ecg_ppg), np.std(all9regtasks_pearson_values_ecg_ppg), np.mean(all9regtasks_mae_values_ecg_ppg), np.std(all9regtasks_mae_values_ecg_ppg)))
    print("10sec -- Mean over all 9 regression tasks -- ECG-only + PPG-only (mean of features) -- Pearson: %.3f +/- %.4f -- MAE: %.3f +/- %.4f" % (np.mean(all9regtasks_pearson_values_ecg_ppg_mean), np.std(all9regtasks_pearson_values_ecg_ppg_mean), np.mean(all9regtasks_mae_values_ecg_ppg_mean), np.std(all9regtasks_mae_values_ecg_ppg_mean)))

    print("########")
    print("10sec -- Mean over the 8 labs regression tasks -- ECG-only -- Pearson: %.3f +/- %.4f -- MAE: %.3f +/- %.4f" % (np.mean(labsregtasks_pearson_values_ecg), np.std(labsregtasks_pearson_values_ecg), np.mean(labsregtasks_mae_values_ecg), np.std(labsregtasks_mae_values_ecg)))
    print("10sec -- Mean over the 8 labs regression tasks -- PPG-only -- Pearson: %.3f +/- %.4f -- MAE: %.3f +/- %.4f" % (np.mean(labsregtasks_pearson_values_ppg), np.std(labsregtasks_pearson_values_ppg), np.mean(labsregtasks_mae_values_ppg), np.std(labsregtasks_mae_values_ppg)))
    print("10sec -- Mean over the 8 labs regression tasks -- ECG + PPG -- Pearson: %.3f +/- %.4f -- MAE: %.3f +/- %.4f" % (np.mean(labsregtasks_pearson_values_ecg_ppg), np.std(labsregtasks_pearson_values_ecg_ppg), np.mean(labsregtasks_mae_values_ecg_ppg), np.std(labsregtasks_mae_values_ecg_ppg)))
    print("10sec -- Mean over the 8 labs regression tasks -- ECG-only + PPG-only (mean of features) -- Pearson: %.3f +/- %.4f -- MAE: %.3f +/- %.4f" % (np.mean(labsregtasks_pearson_values_ecg_ppg_mean), np.std(labsregtasks_pearson_values_ecg_ppg_mean), np.mean(labsregtasks_mae_values_ecg_ppg_mean), np.std(labsregtasks_mae_values_ecg_ppg_mean)))
