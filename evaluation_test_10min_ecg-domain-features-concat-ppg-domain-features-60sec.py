...
save_dir_path = signalmcmed_dir_path + "/evaluation_test_10min_ecg-domain-features-concat-ppg-domain-features-60sec"
...
with open(signalmcmed_dir_path + "/extract-features_ecg-domain-features_10min_ecg.pkl", "rb") as f:
    features_10min_ecg = pickle.load(f)
print(features_10min_ecg.shape)

with open(signalmcmed_dir_path + "/extract-features_ecg-domain-features_csns.pkl", "rb") as f:
    ecg_preprocessed_csns = pickle.load(f)
print(len(ecg_preprocessed_csns))

ecg_preprocessed_csns_filter = []
features_10min_ecg_list = []
for i in range(len(ecg_preprocessed_csns)):
    if ecg_preprocessed_csns[i] in signalmcmed_csns:
        features_10min_ecg_list.append(features_10min_ecg[i])
        ecg_preprocessed_csns_filter.append(ecg_preprocessed_csns[i])
features_10min_ecg = np.stack(features_10min_ecg_list, axis=0)
ecg_preprocessed_csns = ecg_preprocessed_csns_filter
print(features_10min_ecg.shape)
print(len(ecg_preprocessed_csns))
assert len(ecg_preprocessed_csns) == len(signalmcmed_csns)
...
with open(signalmcmed_dir_path + "/extract-features_ppg-domain-features-60sec_csns.pkl", "rb") as f:
    ppg_preprocessed_csns = pickle.load(f)
print(len(ppg_preprocessed_csns))

ppg_preprocessed_csns_filter = []
features_10min_ppg_list = []
for i in range(len(ppg_preprocessed_csns)):
    if ppg_preprocessed_csns[i] in signalmcmed_csns:
        features_10min_ppg_list.append(features_10min_ppg[i])
        ppg_preprocessed_csns_filter.append(ppg_preprocessed_csns[i])
features_10min_ppg = np.stack(features_10min_ppg_list, axis=0)
ppg_preprocessed_csns = ppg_preprocessed_csns_filter
print(features_10min_ppg.shape)
print(len(ppg_preprocessed_csns))
assert len(ppg_preprocessed_csns) == len(signalmcmed_csns)


csn_to_feature_vector_ecg = {}
for i in range(len(ecg_preprocessed_csns)):
    csn_to_feature_vector_ecg[ecg_preprocessed_csns[i]] = features_10min_ecg[i]

csn_to_feature_vector_ppg = {}
for i in range(len(ppg_preprocessed_csns)):
    csn_to_feature_vector_ppg[ppg_preprocessed_csns[i]] = features_10min_ppg[i]

features_10min_ecg_list = []
features_10min_ppg_list = []
features_10min_ecg_ppg_list = []
for csn in signalmcmed_csns:
    features_10min_ecg_list.append(csn_to_feature_vector_ecg[csn])
    features_10min_ppg_list.append(csn_to_feature_vector_ppg[csn])
    feature_vector_ecg_ppg = np.concatenate((csn_to_feature_vector_ecg[csn], csn_to_feature_vector_ppg[csn]))
    features_10min_ecg_ppg_list.append(feature_vector_ecg_ppg)
features_10min_ecg = np.stack(features_10min_ecg_list, axis=0)
features_10min_ppg = np.stack(features_10min_ppg_list, axis=0)
features_10min_ecg_ppg = np.stack(features_10min_ecg_ppg_list, axis=0)
preprocessed_csns = signalmcmed_csns
print(features_10min_ecg.shape)
print(features_10min_ppg.shape)
