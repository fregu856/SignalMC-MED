...
chronos_pipeline = ChronosBoltPipeline.from_pretrained("amazon/chronos-bolt-small", device_map="cuda")
...
for i, csn in enumerate(csns):
    ...
    for batch_i, ecgs in enumerate(loader):
        ecgs = ecgs.cuda().squeeze(1) # (shape: (batch_size, 2500))
        # print(ecgs.shape)
        with torch.no_grad():
            hidden_states, _ = chronos_pipeline.embed(ecgs)   # (shape: [batch_size, seq_len_patched + 1, d_model] == [batch_size, 128 + 1, 512])
            hidden_states = hidden_states.cpu()
            # print(hidden_states.shape)
            features_ecg = hidden_states[:, :-1, :].mean(axis=1) # (shape: [batch_size, 512])
    ...
    for batch_i, ppgs in enumerate(loader):
        ppgs = ppgs.cuda().squeeze(1) # (shape: (batch_size, 2500))
        # print(ppgs.shape)
        with torch.no_grad():
            hidden_states, _ = chronos_pipeline.embed(ppgs)   # (shape: [batch_size, seq_len_patched + 1, d_model] == [batch_size, 128 + 1, 512])
            hidden_states = hidden_states.cpu()
            # print(hidden_states.shape)
            features_ppg = hidden_states[:, :-1, :].mean(axis=1) # (shape: [batch_size, 512])
...
with open(signalmcmed_dir_path + "/extract-features_chronos-bolt-small_csns.pkl", "wb") as f:
    pickle.dump(preprocessed_csns, f)

with open(signalmcmed_dir_path + "/extract-features_chronos-bolt-small_10min_ecg.pkl", "wb") as f:
    pickle.dump(features_10min_ecg, f)
with open(signalmcmed_dir_path + "/extract-features_chronos-bolt-small_5min_ecg.pkl", "wb") as f:
    pickle.dump(features_5min_ecg, f)
with open(signalmcmed_dir_path + "/extract-features_chronos-bolt-small_2min_ecg.pkl", "wb") as f:
    pickle.dump(features_2min_ecg, f)
with open(signalmcmed_dir_path + "/extract-features_chronos-bolt-small_1min_ecg.pkl", "wb") as f:
    pickle.dump(features_1min_ecg, f)
with open(signalmcmed_dir_path + "/extract-features_chronos-bolt-small_30sec_ecg.pkl", "wb") as f:
    pickle.dump(features_30sec_ecg, f)
with open(signalmcmed_dir_path + "/extract-features_chronos-bolt-small_10sec_ecg.pkl", "wb") as f:
    pickle.dump(features_10sec_ecg, f)

with open(signalmcmed_dir_path + "/extract-features_chronos-bolt-small_10min_ppg.pkl", "wb") as f:
    pickle.dump(features_10min_ppg, f)
with open(signalmcmed_dir_path + "/extract-features_chronos-bolt-small_5min_ppg.pkl", "wb") as f:
    pickle.dump(features_5min_ppg, f)
with open(signalmcmed_dir_path + "/extract-features_chronos-bolt-small_2min_ppg.pkl", "wb") as f:
    pickle.dump(features_2min_ppg, f)
with open(signalmcmed_dir_path + "/extract-features_chronos-bolt-small_1min_ppg.pkl", "wb") as f:
    pickle.dump(features_1min_ppg, f)
with open(signalmcmed_dir_path + "/extract-features_chronos-bolt-small_30sec_ppg.pkl", "wb") as f:
    pickle.dump(features_30sec_ppg, f)
with open(signalmcmed_dir_path + "/extract-features_chronos-bolt-small_10sec_ppg.pkl", "wb") as f:
