...
for i, csn in enumerate(csns):
    ...
    # print(ecg_signal_preprocessed.shape)
    ecg_10sec_segments = ecg_signal_preprocessed.view(60, 1000) # (shape: [60, 1000])
    # print(ecg_10sec_segments.shape)
    ecg_10sec_segments = ecg_10sec_segments.unsqueeze(1) # (shape: [60, 1, 10000])
    # print(ecg_10sec_segments.shape)
    #
    ...
    for batch_i, ecgs in enumerate(loader):
        ecgs = ecgs.cuda().squeeze(1) # (shape: (batch_size, 1000))
        # print(ecgs.shape)
        with torch.no_grad():
            signals = torch.zeros((ecgs.shape[0], ecgs.shape[1], 12), dtype=ecgs.dtype).cuda() # (shape: [batch_size, 1000, 12]])
    ...
    # print(ppg_signal_preprocessed.shape)
    ppg_10sec_segments = ppg_signal_preprocessed.view(60, 1000) # (shape: [60, 1000])
    # print(ppg_10sec_segments.shape)
    ppg_10sec_segments = ppg_10sec_segments.unsqueeze(1) # (shape: [60, 1, 10000])
    # print(ppg_10sec_segments.shape)
    #
    ...
    for batch_i, ppgs in enumerate(loader):
        ppgs = ppgs.cuda().squeeze(1) # (shape: (batch_size, 1000))
        # print(ppgs.shape)
        with torch.no_grad():
            signals = torch.zeros((ppgs.shape[0], ppgs.shape[1], 12), dtype=ppgs.dtype).cuda() # (shape: [batch_size, 1000, 12]])
...
with open(signalmcmed_dir_path + "/extract-features_xecg_csns.pkl", "wb") as f:
    pickle.dump(preprocessed_csns, f)

with open(signalmcmed_dir_path + "/extract-features_xecg_10min_ecg.pkl", "wb") as f:
    pickle.dump(features_10min_ecg, f)
with open(signalmcmed_dir_path + "/extract-features_xecg_5min_ecg.pkl", "wb") as f:
    pickle.dump(features_5min_ecg, f)
with open(signalmcmed_dir_path + "/extract-features_xecg_2min_ecg.pkl", "wb") as f:
    pickle.dump(features_2min_ecg, f)
with open(signalmcmed_dir_path + "/extract-features_xecg_1min_ecg.pkl", "wb") as f:
    pickle.dump(features_1min_ecg, f)
with open(signalmcmed_dir_path + "/extract-features_xecg_30sec_ecg.pkl", "wb") as f:
    pickle.dump(features_30sec_ecg, f)
with open(signalmcmed_dir_path + "/extract-features_xecg_10sec_ecg.pkl", "wb") as f:
    pickle.dump(features_10sec_ecg, f)

with open(signalmcmed_dir_path + "/extract-features_xecg_10min_ppg.pkl", "wb") as f:
    pickle.dump(features_10min_ppg, f)
with open(signalmcmed_dir_path + "/extract-features_xecg_5min_ppg.pkl", "wb") as f:
    pickle.dump(features_5min_ppg, f)
with open(signalmcmed_dir_path + "/extract-features_xecg_2min_ppg.pkl", "wb") as f:
    pickle.dump(features_2min_ppg, f)
with open(signalmcmed_dir_path + "/extract-features_xecg_1min_ppg.pkl", "wb") as f:
    pickle.dump(features_1min_ppg, f)
with open(signalmcmed_dir_path + "/extract-features_xecg_30sec_ppg.pkl", "wb") as f:
    pickle.dump(features_30sec_ppg, f)
with open(signalmcmed_dir_path + "/extract-features_xecg_10sec_ppg.pkl", "wb") as f:
